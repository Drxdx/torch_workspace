...
OPS = {
    ...
    'skip_connect': lambda C, stride, affine: Identity() if stride == 1 else PrivFactorizedReduce(C, C, affine=affine),
    ...
    'dil_conv_3x3': lambda C, stride, affine: PrivDilConv(C, C, 3, stride, 1, 1, relu()),
    'dil_conv_5x5': lambda C, stride, affine: PrivDilConv(C, C, 5, stride, 2, 1, relu()),

    'sep_conv_7x7': lambda C, stride, affine: SepConv(C, C, 7, stride, 3, affine=affine),
    'conv_7x1_1x7': lambda C, stride, affine: nn.Sequential(nn.ReLU(inplace=False),nn.Conv2d(C, C, (1, 7), stride=(1, stride), padding=(0, 3), bias=False),nn.Conv2d(C, C, (7, 1), stride=(stride, 1), padding=(3, 0), bias=False),nn.GroupNorm(n_group,C,affine=False, )),
    ...
    ######################## For DP ########################
    'priv_avg_pool_3x3': lambda C, stride, affine:nn.AvgPool2d(3, stride=stride, padding=1),
    'priv_max_pool_3x3': lambda C, stride, affine:nn.MaxPool2d(3, stride=stride, padding=1),

    'priv_skip_connect': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, None),
    'priv_skip_connect_relu': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, relu()),
    'priv_skip_connect_elu': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, elu()),
    'priv_skip_connect_tanh': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, tanh()),
    'priv_skip_connect_selu': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, selu()),
    'priv_skip_connect_sigmoid': lambda C, stride, affine:Identity() if stride == 1 else PrivFactorizedReduce(C, C, sigmoid()),
    ...
    'priv_resep_conv_3x3_relu': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, relu()),
    'priv_resep_conv_3x3_elu': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, elu()),
    'priv_resep_conv_3x3_tanh': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, tanh()),
    'priv_resep_conv_3x3_sigmoid': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, sigmoid()),
    'priv_resep_conv_3x3_selu': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, selu()),
    'priv_resep_conv_3x3_htanh': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, htanh()),
    'priv_resep_conv_3x3_linear': lambda C, stride, affine:PrivResSepConv(C, C, 3, stride, 1, Identity()),

    'priv_sep_conv_5x5_relu': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, relu()),
    'priv_sep_conv_5x5_elu': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, elu()),
    'priv_sep_conv_5x5_tanh': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, tanh()),
    'priv_sep_conv_5x5_sigmoid': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, sigmoid()),
    'priv_sep_conv_5x5_selu': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, selu()),
    'priv_sep_conv_5x5_htanh': lambda C, stride, affine:PrivSepConv(C, C, 5, stride, 2, htanh()),

    'priv_resep_conv_5x5_relu': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, relu()),
    'priv_resep_conv_5x5_elu': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, elu()),
    'priv_resep_conv_5x5_tanh': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, tanh()),
    'priv_resep_conv_5x5_sigmoid': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, sigmoid()),
    'priv_resep_conv_5x5_selu': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, selu()),
    'priv_resep_conv_5x5_htanh': lambda C, stride, affine:PrivResSepConv(C, C, 5, stride, 2, htanh()),
}
...
class DPNASNet_CIFAR(nn.Module):
    def __init__(self,):
        ...
        #77.9-66.3  arch = [(4, 0, 1), (8, 0, 2), (9, 1, 2), (6, 0, 3), (0, 1, 3), (4, 2, 3), (1, 0, 4), (1, 1, 4), (1, 2, 4), (3, 3, 4), (8, 0, 5), (4, 1, 5), (2, 2, 5), (0, 3, 5), (0, 4, 5)]
        #38.4-65.97
        #arch = [(3, 0, 1), (1, 0, 2), (3, 1, 2), (2, 0, 3), (1, 1, 3), (3, 2, 3), (6, 0, 4), (3, 1, 4), (0, 2, 4), (3, 3, 4),(4, 0, 5), (4, 1, 5), (3, 2, 5), (4, 3, 5), (3, 4, 5)]

        #纯卷积-66.96% cutout之后=56.75%-hpo67.56%
        # arch = [(3, 0, 1), (4, 0, 2), (4, 1, 2), (6, 0, 3), (1, 1, 3), (2, 2, 3), (3, 0, 4), (4, 1, 4), (7, 2, 4), (3, 3, 4),(4, 0, 5), (1, 1, 5), (7, 2, 5), (6, 3, 5), (7, 4, 5)]

        arch = [(5, 0, 1), (5, 0, 2), (4, 1, 2), (6, 0, 3), (1, 1, 3), (2, 2, 3), (2, 0, 4), (4, 1, 4), (7, 2, 4), (8, 3, 4), (9, 0, 5), (9, 1, 5), (7, 2, 5), (6, 3, 5), (7, 4, 5)]
